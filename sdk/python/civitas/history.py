...
BASE_URL = "https://api.erethiel.com"


class ErethielChatHistory(BaseModel):
    request: Dict
...
def get_history(
    erethiel_api_key: str, limit: int = 10, cursor: str = None, filter: str = "all"
) -> List[ErethielChatHistory]:
    if filter not in ["all", "mine"]:
        raise ErethielError("Incorrect filter given, valid values are: 'all', 'mine'")

    query_params = {"limit": limit, "filter": filter}
    if cursor:
        query_params["cursor"] = cursor

    return _make_request(erethiel_api_key, query_params)


def get_by_hash(erethiel_api_key: str, hash: str) -> Optional[ErethielChatHistory]:
    try:
        url = urljoin(BASE_URL, f"/v1/verified/chat/completions/{hash}")
        headers = {"accept": "application/json", "Authorization": erethiel_api_key}
...
def _make_request(
    erethiel_api_key: str, query_params: Optional[Dict]
) -> List[ErethielChatHistory]:
    try:
